...
public class RandomTraceableGraphTest {
	...
	public void testRandomGraphTrace() {
		range(0, numberOfGraphs).forEach(i -> {
			...
			Journal journal = new Journal(new InMemoryStore(), journalBlockSize);

			range(0, numberOfEvents).forEach(j -> {
				List<StateTrace> traces = graph.applyTrace(generator.randomEvent());
	...
	private void forwardTrace(Journal journal) {
		Optional<TraceableGraph> g = TraceableGraph.empty(journal)
													.moveAt(0);
