...
public class GraphTest extends GraphTestBase {
	...
	NamedState derived_nodata = new NamedState(NamedState.DERIVED, State.NO_DATA);
	...
	StateChanged comp1_own_alerted = new StateChanged(COMP1, new NamedState(NamedState.OWN, State.ALERT));
	StateChanged comp1_own_clear = new StateChanged(COMP1, own_clear);
	StateChanged comp1_own_warn = new StateChanged(COMP1, own_warn);
	
	StateChanged comp1_derived_alerted = new StateChanged(COMP1, derived_alerted);
	...
	StateChanged comp2_own_alerted = new StateChanged(COMP2, new NamedState(NamedState.OWN, State.ALERT));
	
	StateChanged comp2_derived_warn = new StateChanged(COMP2, derived_warn);
	StateChanged comp2_derived_alerted = new StateChanged(COMP2, derived_alerted);
	
	StateChanged comp3_derived_alerted = new StateChanged(COMP3, derived_alerted);
	...
	public void testAddComponent() {	
		...
		assertEquals(1, graph.stream()
				 .filter(c -> of(COMP1).anyMatch(name -> c.name().equals(name)))
				 .count());
	...
	public void testAdd2Components() {
		...
		assertEquals(2, graph.stream()
				 .filter(c -> of(COMP1, COMP2).anyMatch(name -> c.name().equals(name)))
				 .count());
	}
	...
	public void testStateChanged() {
		...
		verifyStates(graph.get(COMP1), State.NO_DATA, State.NO_DATA, Stream.empty());
		
		assertFalse(comp1.getCheckStated(STATE1).isPresent());
		
		applyTraceAndVerifyImpact(comp1_state1_alerted, 
				Stream.of(
						Stream.of(comp1_state1_alerted), 
						Stream.of(comp1_own_alerted, comp1_derived_alerted)));
		
		verifyStates(graph.get(COMP1), State.ALERT, State.ALERT, Stream.of(state1_alerted));
	}
	...
	public void testOwnStateHighesOfAllCheckedNodata() {
		...
		applyTraceAndVerifyImpact(comp1_state1_nodata, 
				Stream.of(
						Stream.of(comp1_state1_nodata)));

		verifyStates(graph.get(COMP1), State.NO_DATA, State.NO_DATA, Stream.of(state1_nodata));		
	}
	...
	public void testOwnStateHighesOfAllCheckedClear() {
		...
		applyTraceAndVerifyImpact(comp1_state1_clear, 
				Stream.of(
						Stream.of(comp1_state1_clear),
						Stream.of(comp1_own_clear)));

		verifyStates(graph.get(COMP1), State.CLEAR, State.NO_DATA, Stream.of(state1_clear));		
	}
	...
	public void testOwnStateHighesOfAllCheckedWarn() {
		...
		applyTraceAndVerifyImpact(comp1_state1_warn, 
				Stream.of(
						Stream.of(comp1_state1_warn),
						Stream.of(comp1_own_warn, comp1_derived_warn)));

		verifyStates(graph.get(COMP1), State.WARNING, State.WARNING, Stream.of(state1_warn));		
	}
	...
	public void testOwnStateChangedNoTrace() {
		...
		graph.apply(new StateChanged(COMP1, new NamedState(NamedState.OWN, State.ALERT)));
	...
	public void testAdd2ComponentsAndDependency() {
		...
		Component comp1 = graph.get(COMP1);
		assertEquals(COMP2, comp1.outbound().stream()
											.findFirst().get().name());

		Component comp2 = graph.get(COMP2);
		
		assertEquals(COMP1, comp2.inbound().stream()
										   .findFirst().get().name());
	}
	...
	public void testDerivedStateChanged() {
		...
		verifyStates(graph.get(COMP1), State.ALERT, State.ALERT, Stream.of(state1_alerted));
		verifyStates(graph.get(COMP2), State.NO_DATA, State.ALERT, Stream.empty());
	}
	...
	public void testDerivedStateNotChangedOnClear() {
		...
		verifyStates(graph.get(COMP1), State.CLEAR, State.NO_DATA, Stream.of(state1_clear));
		verifyStates(graph.get(COMP2), State.NO_DATA, State.NO_DATA, Stream.empty());
	}

	@Test
	public void testDerivedStatePropagated() {
		applyTraceAndVerify(addedComp1, addedComp2, addedComp3, dep1to2, dep2to3);

		verifyStates(graph.get(COMP1), State.NO_DATA, State.NO_DATA, Stream.empty());
		verifyStates(graph.get(COMP2), State.NO_DATA, State.NO_DATA, Stream.empty());
		verifyStates(graph.get(COMP3), State.NO_DATA, State.NO_DATA, Stream.empty());
		...
		verifyStates(graph.get(COMP1), State.ALERT, State.ALERT, Stream.of(state1_alerted));
		verifyStates(graph.get(COMP2), State.NO_DATA, State.ALERT, Stream.empty());
		verifyStates(graph.get(COMP3), State.NO_DATA, State.ALERT, Stream.empty());
	}

	@Test
	public void testDerivedStateCyclic() {
		applyTraceAndVerify(addedComp1, addedComp2, addedComp3, dep1to2, dep2to3, dep3to1);
	...
	public void testDerivedIsHighestOfAllStates() {
		applyTraceAndVerify(addedComp1, addedComp2, addedComp3, dep1to3, dep2to3);
		...
		applyTraceAndVerifyImpact(comp2_state1_alerted, 
				Stream.of(
		...
		verifyStates(graph.get(COMP1), State.WARNING, State.WARNING, Stream.of(state1_warn));
		verifyStates(graph.get(COMP2), State.ALERT, State.ALERT, Stream.of(state1_alerted));
		verifyStates(graph.get(COMP3), State.NO_DATA, State.ALERT, Stream.empty());
	}
	...
	public void testRemoveConnectedComponent() {
		...
		applyTraceAndVerifyImpact(removedComp2, 
				Stream.of(
						Stream.of(dep1to2removed, dep2to3removed),
