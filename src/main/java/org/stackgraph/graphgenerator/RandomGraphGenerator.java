...
public class RandomGraphGenerator {
	...
	private static final List<State> allStateValues = of(State.NO_DATA,
			State.ALERT, State.CLEAR, State.WARNING).collect(toList());
	...
	public RandomGraphGenerator(int vertexes, int edgeConnectivity,
			int numberOfStates) {
	...
	public Graph generate() {
		...
		IntStream.range(0, vertexes).forEach(
				i -> graph.applyTrace(new ComponentAdded(valueOf(i))));
		IntStream.range(0, vertexes).forEach(
				from -> {
					random.ints(0, vertexes)
							...
							.forEach(
									t -> graph.applyTrace(new DependencyAdded(
											valueOf(from), valueOf(t))));
				});
		...
		IntStream.range(0, vertexes).forEach(
				from -> {
					random.ints(0, numberOfStates)
							...
							.forEach(
									s -> graph
											.applyTrace(new StateChanged(
													valueOf(from),
													new NamedState(valueOf(s),
															State.NO_DATA))));
				});
		return graph;
	}
	...
	private Optional<GraphEvent> stateChanged() {
		List<Component> componentWithStates = graph.stream()
				.filter(c -> c.checkedStates().count() > 0).collect(toList());
		if (componentWithStates.size() > 0) {
			Component randomComponent = randomComponent(componentWithStates
					.stream());
			...
			return Optional.of(new StateChanged(randomComponent.name(),
					new NamedState(randomState.name(), randomState.value(),
							randomNewStateValue)));
		} else {
	...
	private State randomNewState(NamedState randomState) {
		List<State> filtered = allStateValues.stream()
				.filter(s -> s != randomState.value()).collect(toList());
	...
	private NamedState randomState(Component randomComponent) {
		List<NamedState> states = randomComponent.checkedNamedStates().collect(
				toList());
	...
	private Optional<GraphEvent> dependencyRemoved() {
		List<Object> removed = Stream
				.concat(graph.stream().flatMap(
						to -> to.inbound()
								.stream()
								.map(from -> new DependencyRemoved(from.name(),
										to.name()))),
						graph.stream().flatMap(
								from -> from
										.outbound()
										.stream()
										.map(to -> new DependencyRemoved(from
												.name(), to.name()))))
				.distinct().collect(toList());
		if (removed.size() > 0) {
			return Optional.of((GraphEvent) removed.get(random.nextInt(removed
					.size())));
		} else {
			return Optional.empty();
		}
	...
	private Optional<GraphEvent> dependencyAdded() {
		...
		List<Component> chosen = graph
				.stream()
				.filter(c -> {
					return !c.inbound().contains(comp)
							&& !c.outbound().contains(comp) && !c.equals(comp);
				}).collect(toList());
		if (chosen.size() > 0) {
			Component randomComponent = chosen
					.get(random.nextInt(chosen.size()));
			return Optional.of(new DependencyAdded(comp.name(), randomComponent
					.name()));
		} else {
			return Optional.empty();
		}
